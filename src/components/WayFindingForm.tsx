...
const WayFindingForm: React.FC<WayFindingFormProps> = ({
  setPoints,
  component,
  mapData,
}) => {
  ...
  const getFilteredLabels = (inputValue: string) => {
    return spaces.filter(
      (space: any) =>
        ...
        space.id.toLowerCase().includes(inputValue.toLowerCase())
    );
  };

  const handleLabelSelect = (index: number, space: any) => {
    setShowfiltered((prev) => {
      const newShowfiltered = [...prev];
    ...
    setInputValues((prevValues) => {
      const newValues = [...prevValues];
      newValues[index] = space.id; // Set `space.id` properly
      return newValues;
    });
  };
    ...
    <div className="bg-white w-80 z-50 shadow-lg rounded-lg p-5 max-sm:w-[70vw]">
      ...
      >
        ...
        <div className="relative">
          ...
          <div
            className={`mt-2 absolute bg-white w-full shadow-2xl rounded-xl h-20 overflow-y-scroll z-50 ${
              !(inputValues[0] && showfiltered[0]) && "hidden"
            }`}
          >
            {getFilteredLabels(inputValues[0]).map((space:any, index:any) => (
              <div
                key={index}
                className="cursor-pointer p-2 hover:bg-gray-200"
                onClick={() => handleLabelSelect(0, space)} // Pass the object correctly
              >
                {space.id}
              </div>
            ))}
        ...
        <div className="relative">
          ...
          />
          <div
            className={`mt-2 absolute bg-white overflow-hidden w-full shadow-2xl rounded-xl h-20 overflow-y-scroll z-50 ${
              !(inputValues[1] && showfiltered[1]) && "hidden"
            }`}
          >
            {inputValues[1] &&
              showfiltered[1] &&
              getFilteredLabels(inputValues[1]).map((label:any, index:any) => (
                <div
                  key={index}
                  className="cursor-pointer p-2 hover:bg-gray-200"
        ...
        <div className={`${component && "hidden"}`}>
          ...
          <div
            className={`mt-2 absolute bg-white overflow-hidden w-full shadow-2xl rounded-xl h-20 overflow-y-scroll z-50 ${
              !(inputValues[2] && showfiltered[2]) && "hidden"
            }`}
          >
            {inputValues[2] &&
              showfiltered[2] &&
              getFilteredLabels(inputValues[2]).map((label:any, index:any) => (
                <div
                  key={index}
                  className="cursor-pointer p-2 hover:bg-gray-200"
